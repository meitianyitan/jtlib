...
export function getProjectConfig(currentProjectCwd = projectCwd) {
  ...
  const runReactPath = path.join(currentProjectCwd, 'jtlib.config.json')
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    if (packageJson['jtlib']) {
      projectConfig = _.merge(projectConfig, packageJson['jtlib'])
    }
  }

  if (!projectConfig) {
    throw Error(`add jtlib to package.json or create 'jtlib.config.json' in root directory.`)
  }
